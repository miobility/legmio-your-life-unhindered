  ...
  return (
    <div style={{ backgroundColor: INK }}>
            ...
            <Carousel label={tr("Témoignages", "Testimonials", "Erfahrungsberichte")}>
              {[
                ...
                <div key={i} className="fade-up card-white p-6 h-full flex flex-col" style={{ transitionDelay: `${i * 60}ms` }}>
                  ...
                  <div>
                    {t.img && (
                      <Image src={t.img} alt={t.n} className="w-16 h-16 rounded-full object-cover mb-2" loading="lazy" width={64} height={64} />
                    )}
                    <div>
      ...
      {/* 7 — ILS L'ATTENDENT DEJA (INK)
          ...
          plus haut, a part. */}
      <section style={{ backgroundColor: INK }} className="grain relative jonction-haut jonction-bas halo-or px-4 sm:px-6 py-20 md:py-28 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <Reveal>
...
function WallOfLove() {
  ...
  const Card = ({ q }: { q: string }) => (
    <div className="rounded-xl px-6 py-4 shrink-0 max-w-xs" style={{ backgroundColor: WHITE, border: `1px solid ${LINE_INK}`, color: INK, boxShadow: "0 1px 2px rgba(16,38,58,0.06)" }}>
